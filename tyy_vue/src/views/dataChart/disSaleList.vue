<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column  align="center"  min-width="20" label="分销商头像">
        <template slot-scope="scope">
          <el-popover placement="right" v-if="scope.row.disImgs&&scope.row.disImgs.length>0">
            <div slot="reference">
              <el-image :src="scope.row.disImgs[0]" style="width: 100px;height: 50px;"
                        v-if="scope.row.disImgs.length===1"></el-image>
              <el-button type="text" v-else>共{{scope.row.disImgs.length}}张</el-button>
            </div>
            <div style="max-width: 85vw;max-height: 75vh">
              <el-image :src="item" style="width: 350px;height: 240px;margin: 25px;" :key="item"
                        v-for="item in scope.row.disImgs"></el-image>
            </div>
          </el-popover>
      ...
      <el-table-column align="center" label="分销商单位名称" width="150">
        <template slot-scope="scope">
          <el-button @click="$router.push({name:'DisList',params:{disName:scope.row.disName}})" type="text">
            {{scope.row.disName}}
          </el-button>
          <el-tag v-if="scope.row.disInfo.isBlack===1" type="info">赖账</el-tag>
        </template>
      </el-table-column>

      <el-table-column align="center" label="累计销售额（元）" min-width="10">
        <template slot-scope="scope">
...
<script>
    ...
    import {getDisSales} from "@/api/distribute";
    ...
    export default {
        ...
        methods: {
            getList() {
                getDisSales(this.queryForm).then(res => {
