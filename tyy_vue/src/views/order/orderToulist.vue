<template>
  <div style="min-height: 130px;">
    ...
    <el-table :data="list.slice((query.currentPage - 1) * query.pageSize, query.currentPage * query.pageSize)" border
              ref="mtable"
              stripe>
      <el-table-column type="selection"></el-table-column>

      <el-table-column align="center" label="游客类型">
        <template slot-scope="scope">{{scope.row.touType}}</template>
      </el-table-column>
      <el-table-column align="center" label="游客姓名">
        <template slot-scope="scope">{{scope.row.touName}}</template>
      </el-table-column>
      <el-table-column align="center" label="游客性别">
        <template slot-scope="scope">
          <el-tag type="success" v-if="scope.row.touSex===1">男</el-tag>
          <el-tag type="warning" v-if="scope.row.touSex===0">女</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="证件类型">
        <template slot-scope="scope">{{scope.row.cerTypeName}}</template>
      </el-table-column>
