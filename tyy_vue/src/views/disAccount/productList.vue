      ...
      <el-table-column align="center" label="操作" min-width="25">
        <template slot-scope="scope">
          ...
          <div style="margin-top: 5px;">
            ...
            <el-button  @click="$router.push({name:'ProductInfo',query:{id:scope.row.id}})" size="mini" type="success">
              录入订单
            </el-button>
...
<script>
  import {delProduct, getAllPros, getProTye, getSerTypes,getLocations,getProTicList} from "@/api/production"
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        this.onLoading = true
        getAllPros(this.queryForm).then(res => {
          this.list = res.list
