<template>
	<view>
		...
		<uni-section title="现在团队" type="line"/>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">团队名称</view>
					<view style="width: 70%;">{{groupNames}}</view>
				</view>
				<view class="bottomLine"/>
		</view>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">发团日期</view>
					<view style="width: 70%;">{{groupDate}}</view>
				</view>
				<view class="bottomLine"/>
		</view>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">导游姓名</view>
					<view style="width: 70%;">{{guiderName}}</view>
				</view>
				<view class="bottomLine"/>
		</view>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">车辆名称</view>
					<view style="width: 70%;">{{carName}}</view>
				</view>
				<view class="bottomLine"/>
		</view>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">车牌号</view>
					<view style="width: 70%;">{{carNumber}}</view>
				</view>
				<view class="bottomLine"/>
		</view>
		<uni-section title="修改团队" type="line"/>
		<view class="adBaseView">
				<view class="adRowView">
					<view class="headView">团队名称</view>
		...
		<view class="adBaseView">
			<view class="adRowView">
				<view class="headView">修改发团日期(请慎重修改)</view>
				<view >
					<picker mode="date" :value="groupDate" :start="startDate" :end="endDate" @change="bindDateChange">
					                        <view>{{groupDate}}</view>
					                    </picker>
				</view>
			</view>
		...
		<button type="primary" @click="submit">修改团队</button>
		</view>
</template>
...
<script>
	...
	export default {
		...
		getGuider().then(res => {
			...
			edityfGroup({gruopId:this.groupId}).then(res => {
				this.guiderName = res.data.guiderName
				this.carNumber = res.data.carNumber
				this.carName = res.data.carName
				this.groupNames = res.data.groupName
				this.groupDate = res.data.groupDate
			});
		},
		computed: {
		       startDate() {
			...
			submit(){
				if(this.groupName === ''||this.value1 === ''||this.value2 === ''||this.value3 === ''||this.value4 === ''){
					uni.showModal({
						...
						submitModifyGroupDate({guiderId:this.value2,carId:this.value4,groupName:this.groupName,groupId: this.groupId,groupDate: this.groupDate}).then(res => {
					uni.showModal({
					    title: '提示',
						showCancel: false,
						confirmColor: "#000000",
