						...
						<view class="user like_">
							<view class="user_say like_o">
								<view class="user_head like_h">
									...
									<text class="doc_time time_">{{ item.locationAddress }}</text>
								</view>
								<view class="user_talk like_talk">
									<text>{{ item.content }}</text>
									<view class="like_img" ><image v-for="pic in item.photoData" @tap="_previewImage(pic.photo)" :src="pic.photo"></image></view>
								</view>
							</view>
						</view>
					</view>
...
<script>
import { otherViewJournal } from '../../api/guider/guider.js';
...
export default {
	...
	onLoad(options) {
		...
		otherViewJournal({groupId:this.groupId}).then(res => {
								if(res.re==1){
