	...
	<view>
		...
		<view class="form-item"  >			
			<view style="display : flex ; flex-flow : row;">
					...
					<view  style="width: 300upx;color: blue;text-align: right;" >
						<button type="primary" style="width: 70%;" @click="submit" >提交</button>
					</view>
		</view>			
		</view>
		</view>
...
<script>
import { getPlanDate, getLocation, viewTourist,othersAddOrder,getPort ,savaIdentity,viewTouristAndPortList,getPlanNum} from '../../api/tour/tour.js';
export default {
	data() {
		return {
			orderId:'',
	...
	methods: {
		getLocation: function(item) {
			...
			getLocation({ id: item.id })
				...
				.catch(err => {});
			getPort({ planId: item.id,disId:this.disId })
				.then(res => {
		...
		submit :function(){
			othersAddOrder({tourList:this.tourList,planId: this.planId,location:this.location,date : this.date,touristIds:this.touristIds,disId:this.disId,staffId:this.staffId,productId:this.productId})
				.then(res => {
					this.orderId = res.data
										if(res.status==1){
	                   uni.navigateTo({
											url: './otherOrderDetail?orderId='+this.orderId+'&tourList='+encodeURIComponent(JSON.stringify(this.tourList))+'&touristIds='+encodeURIComponent(JSON.stringify(this.touristIds))+'&region='+this.region+'  '
										})	
					}else{
					uni.showModal({
					    title: '提示',
