<template>
	<view class="container">
		...
		<view class="carousel"  v-for="(ima,index) in item.images" :key="index">
	...
	export default{
		...
		onLoad(options){
			...
			tourProductInit({productId:this.productId}).then(res => {
						...
						statisticsViews({productId:this.productId,code:this.code}).then(res => {
			...
			buy(){
				tourCanbyInit().then(res => {
					if(res.data==='请完善个人信息'){
						...
						uni.navigateTo({
							url: '../product/pay?productId='+this.productId+''
						})
					}
